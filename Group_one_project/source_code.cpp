...
string server = "FBABD\\SQLEXPRESS"; // Escape backslash
string database = "projectccc2";
string username = "sa";
string password = "abdre4777";
    ...
    do
    {
        ...
        for (int i = 0; i < h.size(); i++)
        {
            if (h[i].city_code == city_code && h[i].subcity_code == subcity_code && h[i].price <= price)
            {
                ...
                cout << " ********** " << "      Availablity: " << h[i].avilablity << endl;
                x = false;
            }
        }
        if (x)
...
void fetchBusinesses(vector<bussiness> &bs)
{
   string query = "sqlcmd -S " + server + " -d " + database + " -U " + username + " -P " + password + " -W -s\\t -Q \"SET NOCOUNT ON; SELECT ID, buss_name, name, password, city, phone_num FROM bussiness\"";
    ...
    while (getline(stream, line)) {
        // Skip lines that are empty or contain just formatting
        if (line.empty() || line.find("---") != string::npos || line.find("rows affected") != string::npos) continue;

        bussiness b;
        istringstream lineStream(line);
        
        // Parsing based on tab separation
        lineStream >> b.ID >> b.buss_name >> b.name >> b.password >> b.city >> b.phone_num;

        // Debug: Output each parsed line
        // cout << "Parsed line: " << b.ID << " " << b.buss_name << " " << b.name << " " << b.password << " " << b.city << " " << b.phone_num << endl;
...
first_page:
    ...
    case '1':
        ...
        customer_home_page:
            ...
            {
            case '1':
                cout << "Which house (1,2,3 or 4) based on the order " << endl;
                cin >> choose;
